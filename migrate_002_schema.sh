...
with engine.begin() as conn:
    ...
    # ------------------------------------------------------------------
    # movies table: add imdb_id column (stores OMDb imdbID for direct linking)
    # ------------------------------------------------------------------
    if not column_exists(conn, "movies", "imdb_id"):
        print("Adding column movies.imdb_id...")
        conn.execute(text("ALTER TABLE movies ADD COLUMN imdb_id VARCHAR(20) NULL AFTER id"))
    else:
        print("column movies.imdb_id: already exists, skipping.")

print("")
print("Migration complete. Final schema (ai_filtered_views):")
with engine.connect() as conn:
    result = conn.execute(text("DESCRIBE ai_filtered_views"))
    for row in result:
        print(f"  {row[0]:20s} {row[1]:25s} null={row[2]} key={row[3]}")
print("")
print("Final schema (movies — imdb_id column):")
with engine.connect() as conn:
    result = conn.execute(text(
        "SELECT COLUMN_NAME, COLUMN_TYPE, IS_NULLABLE FROM information_schema.COLUMNS "
        "WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'movies' AND COLUMN_NAME = 'imdb_id'"
    ))
    for row in result:
        print(f"  {row[0]:20s} {row[1]:25s} null={row[2]}")
PYEOF
