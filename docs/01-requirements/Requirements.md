...
- **FR-105:** The Movies, Series, News, and Design tabs MUST each repeat their toolbar (view toggles, category/mode toggles, item count, and any Mark All Read / Ignore All / Export controls) below the item list, in addition to the existing copy above it, so the controls remain reachable without scrolling back to the top. The bottom toolbar MUST only render when the list has content — it MUST NOT appear during loading or on an empty list. The Saved tab is out of scope (no view-toggle toolbar to duplicate).
...
- **AC-053:** On Movies, Series, News, and Design, when a list has at least one item, the same toolbar controls (view toggles, category/mode toggles, item count, Mark All Read/Ignore All/Export) appear both above and below the list, and both copies operate on the same state (e.g. toggling Read/Unread from either copy switches the whole view). The bottom copy is absent while loading and on an empty list.
