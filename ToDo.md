1. In the series feed we will keep the "Following" & "Ignored" categories, and add a new one called "Inbox". The user can select and assign the category Following (as we do with "Ignore" now), and/or filter by name. The name filtering uses an specific entry in the "config.yaml" that has the string patterns to filter and add the "Following" flag/category. Additionally the flag/category can be assigned as we do now with the "Ignore". Series that are not marked as "Following" & "Ignored", will go into the Inbox category. DOCS DONE (Requirements FR-081–FR-085/AC-027–AC-031, Data-Contracts, Architecture-Overview, Constraints-Register C-010/F-006, Change-Log) — code implementation still pending, awaiting go-ahead.
